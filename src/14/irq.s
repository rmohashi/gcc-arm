...
do_irq_interrupt:
  str   r0, linhaA              @Store r0 in linhaA
  adr   r0, linhaA + 4          @Store the second position of linhaA in r0
  sub   lr, lr, #4
  stm   r0!, {r1-r12, lr}       @Store registers r1-r12 and lr(pc) in linhaA
  mrs   r14, spsr
  str   r14, [r0], #4           @Store spsr
  mrs   r1, cpsr
  msr   cpsr_ctl, #0b11010011   @Supervisor mode
  stm   r0!, {sp, lr}           @Store sp and lr
  ...
  mrs   r1, cpsr
  msr   cpsr_ctl, #0b11010011   @Supervisor mode
  ldmdb r0!, {sp, lr}           @Restore sp and lr
  msr   cpsr, r1                @Exit supervisor mode
  ldr   r14, [r0, #-4]!
  msr   spsr, r14               @Restore spsr
  ldmdb r0, {r0-r12, pc}^       @Restore r1-r12 and pc registers
  ldmfd sp!,{r0-r3,pc}^
